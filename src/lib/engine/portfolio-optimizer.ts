...
// Model portfolios based on years to target
  ...
  for (let i = 0; i < weights.length; i++) {
    for (let j = 0; j < weights.length; j++) {
      variance +=
        weights[i] *
        weights[j] *
        ASSET_CLASSES[i].volatility *
        ASSET_CLASSES[j].volatility *
        CORRELATIONS[i][j];
    }
  }
...
function recommendProfile(yearsToTarget: number, currentAge: number): RiskProfile {
  // Longer horizon + younger age → more aggressive
...
export function optimizePortfolio(
  ...
  const riskFreeRate = 0.02; // approximate
  ...
  const portfolios: PortfolioAllocation[] = profiles.map((profile) => {
    const weights = models[profile];
    const stats = calculatePortfolioStats(weights);
    const sharpeRatio =
      stats.volatility > 0 ? (stats.expectedReturn - riskFreeRate) / stats.volatility : 0;

    return {
      profile,
      label: labels[profile],
      allocations: ASSET_CLASSES.map((ac, i) => ({
        assetId: ac.id,
        weight: weights[i],
      })),
  ...
  // Project growth for each portfolio
  const maxYears = 80 - input.currentAge;
  const projectedGrowth: PortfolioResult['projectedGrowth'] = [];
  const annualSavings = input.annualIncome * input.savingsRate;

  for (let y = 0; y <= maxYears; y++) {
    const age = input.currentAge + y;
    const entry: PortfolioResult['projectedGrowth'][0] = {
      age,
      conservative: 0,
      moderate: 0,
      aggressive: 0,
    ...
    projectedGrowth.push(entry);
  }

  // Years to FIRE for each portfolio (using Regular FIRE target)
  ...
  const recRealReturn =
    (1 + recommendedPortfolio.expectedReturn) / (1 + input.inflation) - 1;
  ...
  for (const type of ['lean', 'regular', 'fat', 'barista'] as const) {
    ...
    for (let y = 0; y <= maxYears; y++) {
      ...
      nw = nw * (1 + recRealReturn) + annualSavings;
    }
  }
